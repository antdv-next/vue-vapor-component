...
const [getValidTime, rowHourUnits, getMinuteUnits, getSecondUnits, getMillisecondUnits] =
  useTimeInfo(generateConfig, computed(() => props as any), value)
...
const onCellDblClick = computed(() => pickerHackContext?.value?.onCellDblClick)
...
// Minutes
const validHour = computed(() => getEnabled(rowHourUnits.value, hour.value as number))
const minuteUnits = computed(() => getMinuteUnits(validHour.value))

// Seconds
const validMinute = computed(() => getEnabled(minuteUnits.value, minute.value as number))
const secondUnits = computed(() => getSecondUnits(validHour.value, validMinute.value))

// Milliseconds
const validSecond = computed(() => getEnabled(secondUnits.value, second.value as number))
const millisecondUnits = computed(() =>
  getMillisecondUnits(validHour.value, validMinute.value, validSecond.value),
)

const validMillisecond = computed(() => getEnabled(millisecondUnits.value, millisecond.value as number))
...
const triggerDateTmpl = computed(() => {
  let tmpl = value.value || ctx.value.pickerValue || generateConfig.value.getNow()
  const gc = generateConfig.value
...
const onNextMillisecondTime = (val: number) => fillColumnValue(val, 'setMillisecond')
...
const onHourChange = (val: number | string) => triggerChange(onNextHourTime(val as number))
const onMinuteChange = (val: number | string) => triggerChange(onNextMinuteTime(val as number))
const onSecondChange = (val: number | string) => triggerChange(onNextSecondTime(val as number))
const onMillisecondChange = (val: number | string) =>
  triggerChange(onNextMillisecondTime(val as number))
const onMeridiemChange = (val: number | string) => triggerChange(getMeridiemTime(val as string))

const onHourHover = (val: number | string | null) => onHover.value?.(onNextHourTime(val as number))
...
const onMeridiemHover = (val: number | string | null) => onHover.value?.(getMeridiemTime(val as string))
</script>
