...
const cellRender = computed<CellRender<any> | undefined>(() => ctx.value.cellRender)

const cellInfo = (unitValue: number | string): CellRenderInfo<any> => ({
  prefixCls: ctx.value.prefixCls as string,
  originLabel: String(props.units.find(u => u.value === unitValue)?.label ?? ''),
  today: ctx.value.now,
  type: 'time',
  subType: props.type,
...
const cells = computed<TimeColumnCell[]>(() =>
  props.units.map((unit) => {
    ...
    const isComponent = node != null && typeof node !== 'string' && !isVNode(node)
...
<template>
  <ul ref="ul" :class="columnPrefixCls" :data-type="type" @scroll="onInternalScroll">
