...
const disabledSuperOffsetPrev = computed<boolean>(() => {
  ...
  const prevPanelLimitDate = props.getEnd(props.superOffset(-1, ctx.pickerValue))
  return !isSameOrAfter(
    ctx.generateConfig as any,
    ctx.locale as any,
    prevPanelLimitDate,
...
const disabledSuperOffsetNext = computed<boolean>(() => {
  ...
  const nextPanelLimitDate = props.getStart(props.superOffset(1, ctx.pickerValue))
  return !isSameOrAfter(
    ctx.generateConfig as any,
    ctx.locale as any,
    ctx.maxDate,
...
// ======================== Render ========================
const hideHeader = computed<boolean>(() => !!pickerHackContext?.value?.hideHeader)
...
const headerCls = computed(() => clsx(headerPrefixCls.value, panelClassNames.value?.header))
