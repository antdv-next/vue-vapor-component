<script setup vapor lang="ts">
  ...
  const panelPrefixCls = computed(
    () => `${props.prefixCls}-${props.panelName}-panel`,
  )
  const isWeek = computed<boolean>(() => props.mode === 'week')

  const weekFirstDay = computed(
    () =>
      props.generateConfig?.locale?.getWeekFirstDay?.(props.locale!.locale!) ??
      0,
  )
  ...
  const month = computed(() =>
    props.generateConfig?.getMonth?.(props.pickerValue),
  )
  ...
  const prefixColumnText = computed(() =>
    showPrefixColumn.value
      ? (date: any) =>
          props.generateConfig?.locale.getWeek?.(props.locale!.locale, date)
      : undefined,
  )
  ...
  const headerCells = computed<string[]>(() => {
    ...
    for (let i = 0; i < WEEK_DAY_COUNT; i += 1) {
      cells.push(
        weekDaysLocale.value[(i + weekFirstDay.value) % WEEK_DAY_COUNT],
      )
    }
    return cells
  })
  ...
  const getCellDate = (date: any, offset: number) =>
    props.generateConfig?.addDate?.(date, offset)
  ...
  const getCellClassName = (date: any) => {
    ...
    return {
      ...
      [`${props.prefixCls}-cell-today`]: isSameDate(
        props.generateConfig!,
        date,
        nowVal,
      ),
