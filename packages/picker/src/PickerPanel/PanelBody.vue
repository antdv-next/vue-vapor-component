    ...
    result.push({
      ...
      prefixCls: props.prefixColumnText
        ? clsx(cellPrefixCls, `${cellPrefixCls}-week`, {
            [`${cellPrefixCls}-disabled`]: !!props.prefixColumnDisabled?.(rowStartDate),
          })
        : undefined,
      prefixLabel: props.prefixColumnText ? props.prefixColumnText(rowStartDate) : undefined,
    })
  }
...
const bodyCls = computed(() => clsx(`${prefixCls.value}-body`, panelClassNames.value?.body))
      ...
      <thead v-if="headerCells">
        <tr>
          <th v-if="prefixColumnText">
            <span
              style="position: absolute; overflow: hidden; width: 0; height: 0; opacity: 0"
            >
              {{ headerHiddenLabel }}
            </span>
          </th>
