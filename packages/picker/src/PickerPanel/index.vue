...
const triggerChange = (nextValue: any[] | null) => {
  ...
  if (
    ...
    (nextValue === null ||
      ...
      mergedValue.value.some((ori, index) => !isSame(
...
const mergedPickerValue = computed(() =>
  props.pickerValue !== undefined ? props.pickerValue : internalPickerValueState.value,
)
...
if (process.env.NODE_ENV !== 'production') {
  warning(
    !mergedValue.value ||
      mergedValue.value.every(val => mergedGenerateConfig.value.isValidate(val)),
...
const nodeCls = computed(() =>
  clsx(panelCls.value, { [`${panelCls.value}-rtl`]: props.direction === 'rtl' }),
)
...
<template>
  <div ref="root" :tabindex="tabindex" :class="nodeCls" v-bind="attrs">
    <component
      ...
      :on-picker-value-change="(nextPickerValue: any) => setPickerValue(nextPickerValue, true)"
      :value="mergedValue[0]"
