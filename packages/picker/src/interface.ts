...
export interface CellRenderInfo<DateType> extends BaseInfo {
  prefixCls: string
  /**
   * Vapor deviation: the vdom source exposed `originNode: VNode` so a custom
   * `cellRender` could wrap or return the default cell node. Vapor cannot carry a
   * `VNode` around and conditionally render it (skill rules 7.1/7.5/26), so only
   * the cell text is handed over and the default cell markup is rendered by
   * `PickerPanel/CellItem.vue` when `cellRender` returns `null`/`undefined`.
   */
  originLabel: string
  today: DateType
  type: PanelMode
...
/**
 * Vapor deviation: the return type is `Component | string` instead of `VueNode`.
 * Vapor cannot carry a `VNode` around and conditionally render it (skill rules
 * 7.1/7.5/26), so a cell renderer returns a component definition or a tag string,
 * and `PickerPanel/CellItem.vue` renders it with `<component :is>`.
 *
 * `null`/`undefined` renders the default cell markup. `VueNode` is retained only
 * because the deprecated `dateRender`/`monthCellRender` still return `VueNode`;
 * `CellItem.vue` detects those with `isVNode()` and renders them through
 * `vaporInteropPlugin`.
 */
export type CellRender<DateType, CurrentType = DateType | number | string> = (
  current: CurrentType,
  info: CellRenderInfo<DateType>,
) => Component | string | VueNode | null | undefined
...
/**
 * Flat interface listing the HTML attributes the picker exposes.
 * The SFC compiler cannot resolve `Omit<HTMLAttributes, ...>` in an
 * `interface extends` clause (skill rule — see checkbox/select pattern).
 * Attributes that conflict with the picker's own props (value, onChange,
 * disabled, onFocus, onBlur, etc.) are intentionally excluded.
 */
export interface SharedHTMLAttrs {
  title?: string
  lang?: string
  dir?: string
...
export interface SharedPickerProps<DateType extends object = any>
   ...
   */
  // Vapor: `| false` stripped. Skill rule 7/28 — a union containing `false` makes
  // vapor coerce an unpassed `undefined` into `false`, which silently kills the
  // `?? 'hover'` default in `useFilledProps`. `!previewValue` covers both cases.
  previewValue?: PreviewValueType
  ...
  showToday?: boolean
  /**
   * Vapor deviation: the vdom source passed the default panel node as
   * `originPanel` so callers could `cloneElement` it. A vapor node cannot be
   * carried around (skill rules 7.1/7.5/26/32), so the argument is the current
   * panel mode instead and the return value replaces the whole panel body.
   * Vapor 偏差：vdom 版把默认面板节点作为 `originPanel` 传入以便
   * `cloneElement`。vapor 节点无法传递（规则 7.1/7.5/26/32），因此参数改为
   * 当前面板 mode，返回值替换整个面板主体。
   */
  panelRender?: (panelMode: PanelMode) => VueNode
...
// rangePicker
type RangePickerRefBase = Omit<PickerRef, 'focus'>

export interface RangePickerRef extends RangePickerRefBase {
...
type SelectorHTMLAttrs = Omit<SharedHTMLAttrs, 'onClick'>

export interface SelectorProps<DateType = any> extends SelectorHTMLAttrs {
  picker: PickerMode
