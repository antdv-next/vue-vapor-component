...
>(
  ...
  // ======================== Props =========================
  // `cloneProps` instead of `...props.value` (skill rule 18): a vapor props
  // proxy's `getOwnPropertyDescriptor` trap returns `undefined` for
  // emit-listened keys, so the spread operator silently drops every `onXxx`
  // handler (`onChange`, `onOpenChange`, `onClear`, `onOk`, ...).
  // 用 `cloneProps` 替代 `...props.value`（规则 18）：vapor 的 props proxy 对
  // emit 绑定的 key 返回 `undefined`，spread 会静默丢掉所有 `onXxx` 处理器
  // （`onChange`、`onOpenChange`、`onClear`、`onOk` 等）。
  const filledProps = computed(() => ({
    ...cloneProps(props.value as any),
    ...
    clearIcon: fillClearIcon(props.value.allowClear, props.value.clearIcon),
