...
export default function useAlign(
  ...
  const _onAlign = (cache = false) => {
    // Guard against re-entry: _onAlign → reactive offsetInfo → re-render →
    // getBoundingClientRect() changes → useResizeObserver → triggerAlign → _onAlign
    // Use requestAnimationFrame to reset the flag AFTER the entire render+measure cycle.
    // A microtask reset fires too early because useResizeObserver's callback is also a microtask queued later.
