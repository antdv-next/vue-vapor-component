...
const attrsAriaData = computed(() => pickAttrs(attrs, { aria: true, data: true }))
...
const flattenNodes = computed(() => flattenTreeData(
...
const selectedKeys = shallowRef<Key[]>(
  calcSelectedKeys(props.selectedKeys || props.defaultSelectedKeys || [], { multiple: props.multiple }) || [],
)

watchEffect(() => {
  const sk = props.selectedKeys
  if (sk === undefined) return
  selectedKeys.value = calcSelectedKeys(sk, { multiple: props.multiple }) || []
})
...
const scrollTo: TreeRef['scrollTo'] = (scroll) => {
  if (scroll && typeof scroll === 'object' && 'autoExpand' in scroll
    && (scroll as any).autoExpand && props.expandedKeys === undefined) {
...
const onKeyDown: TreeRef['onKeyDown'] = (e: KeyboardEvent) => {
  ...
  if (activeItem && activeItem.data) {
    const eventNode = convertNodePropsToEventData({
      ...getTreeNodeProps(activeKey.value!, getTreeNodeRequiredPropsVal.value),
    ...
    const canCheck = !!props.checkable
      && !eventNode.disabled
      && eventNode.checkable !== false
      && !eventNode.disableCheckbox
    const canSelect = !props.checkable
      && props.selectable
      && !eventNode.disabled
...
let onNodeCheck: (e: MouseEvent, treeNode: any, checked: boolean) => void = () => {}
...
let onNodeDragEnd: (event: DragEvent, nodeProps: any | null, outsideTree?: boolean) => void = () => {}
let onNodeDrop: (event: DragEvent, nodeProps: any | null, outsideTree?: boolean) => void = () => {}
...
const treeCtx = reactive<any>({
  ...
  get prefixCls() { return props.prefixCls },
  get selectable() { return props.selectable },
  get showIcon() { return props.showIcon },
  get icon() { return props.icon },
  get switcherIcon() { return props.switcherIcon },
  get draggable() { return draggableConfig.value },
  get draggingNodeKey() { return draggingNodeKey.value },
  get checkable() { return props.checkable },
  get checkStrictly() { return props.checkStrictly },
  get disabled() { return props.disabled },
  get keyEntities() { return keyEntities.value },
  get dropLevelOffset() { return dropLevelOffset.value },
  get dropContainerKey() { return dropContainerKey.value },
  get dropTargetKey() { return dropTargetKey.value },
  get dropPosition() { return dropPosition.value },
  get indent() { return indent.value },
  dropIndicatorRender: (diProps: any) => {
    if (props.dropIndicatorRender)
      return props.dropIndicatorRender(diProps)
    return null
  },
  get dragOverNodeKey() { return dragOverNodeKey.value },
  get direction() { return props.direction },
  get loadData() { return props.loadData },
  get filterTreeNode() { return props.filterTreeNode },
  get titleRender() { return props.titleRender },
  get allowDrop() { return props.allowDrop! },
  get styles() { return props.styles },
  ...
  get onNodeLoad() { return onNodeLoad },
  get onNodeExpand() { return onNodeExpand },
  get onNodeClick() { return onNodeClick },
  get onNodeDoubleClick() { return onNodeDoubleClick },
  get onNodeSelect() { return onNodeSelect },
  get onNodeCheck() { return onNodeCheck },
  get onNodeMouseEnter() { return onNodeMouseEnter },
  get onNodeMouseLeave() { return onNodeMouseLeave },
  get onNodeContextMenu() { return onNodeContextMenu },
  get onNodeDragStart() { return onNodeDragStart },
  get onNodeDragEnter() { return onNodeDragEnter },
  get onNodeDragOver() { return onNodeDragOver },
  get onNodeDragLeave() { return onNodeDragLeave },
  get onNodeDragEnd() { return onNodeDragEnd },
  get onNodeDrop() { return onNodeDrop },
})
...
onNodeDragStart = (event: DragEvent, nodeProps: any) => {
  ...
  props.onDragStart?.({ event, node: convertNodePropsToEventData(nodeProps) })
}
...
onNodeDragOver = (event: DragEvent, nodeProps: any) => {
  ...
  props.onDragOver?.({ event, node: convertNodePropsToEventData(nodeProps) })
}
...
onNodeDragLeave = (event: DragEvent, nodeProps: any) => {
  ...
  props.onDragLeave?.({ event, node: convertNodePropsToEventData(nodeProps) })
}

onNodeDragEnd = (event: DragEvent, nodeProps: any | null, _outsideTree?: boolean) => {
  dragOverNodeKey.value = null
  cleanDragState()

  if (nodeProps) {
    props.onDragEnd?.({ event, node: convertNodePropsToEventData(nodeProps) })
  }
...
onNodeDrop = (event: DragEvent, _nodeProps: any | null, outsideTree = false) => {
  ...
  const abstractDropNodeProps = {
    ...getTreeNodeProps(dropTargetKeyValue, getTreeNodeRequiredPropsVal.value),
  ...
  const dropResult: any = {
    ...
    dragNode: dragNodePropsValue ? convertNodePropsToEventData(dragNodePropsValue) : null,
...
function onActiveChange(newActiveKey: Key | null) {
  ...
  if (newActiveKey !== null) {
    nextTick(() => {
      listRef.value?.scrollTo?.({ key: newActiveKey, offset: props.itemScrollOffset || 0 })
    })
  }
...
<template>
  ...
  >
    <NodeList
      ...
      :on-list-change-start="onListChangeStart"
      :on-list-change-end="onListChangeEnd"
