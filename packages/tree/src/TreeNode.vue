...
import { TreeContextKey, UnstableContextKey, useTreeContext, useUnstableContext } from './TreeContextKey'
...
const isDisabled = computed(() => {
  const c = ctx.value
  if (!c) return false
  return !!(c.disabled || props.disabled || unstableCtx.value?.nodeDisabled?.(props.data as any))
})
...
const hasChildren = computed(() => {
  const c = ctx.value
  const { children } = c?.keyEntities ? getEntity(c.keyEntities, props.eventKey!) || {} : {}
  return Boolean((children || []).length)
})

const memoizedIsLeaf = computed(() => {
  return isLeafNode(props.isLeaf, ctx.value?.loadData, hasChildren.value, props.loaded)
})

watchEffect(() => {
  if (props.loading) return
  const c = ctx.value
  if (typeof c?.loadData === 'function' && props.expanded && !memoizedIsLeaf.value && !props.loaded) {
...
const level = computed(() => {
  const c = ctx.value
  return (c?.keyEntities ? (getEntity(c.keyEntities, props.eventKey!) || {}).level : 0) || 0
})
const isEndNode = computed(() => !!props.isEnd?.[props.isEnd.length - 1])

const isDraggable = computed(() => {
  const c = ctx.value
  return !!(c?.draggable && (!c.draggable.nodeDraggable || c.draggable.nodeDraggable(props.data as any)))
})
...
const switcherStyle = computed<CSSProperties>(() => ctx.value?.styles?.itemSwitcher || {})
...
const dragHandlerClass = computed(() => `${ctx.value?.prefixCls || 'vc-tree'}-draggable-icon`)
...
const checkboxClass = computed(() => {
  ...
  return clsx(`${c.prefixCls}-checkbox`, {
    ...
    [`${c.prefixCls}-checkbox-indeterminate`]: !props.checked && props.halfChecked,
    [`${c.prefixCls}-checkbox-disabled`]: isDisabled.value || props.disableCheckbox,
  })
})

const customCheckbox = computed(() => {
  if (typeof isCheckable.value !== 'boolean')
    return isCheckable.value
  return null
})

const dataOrAriaAttrProps = computed(() => pickAttrs(attrs, { aria: true, data: true }))

const treeCls = computed(() => {
  const c = ctx.value
  if (!c) return ''
  return clsx(props.className, `${c.prefixCls}-treenode`, c.classNames?.item, {
    [`${c.prefixCls}-treenode-disabled`]: isDisabled.value,
    [`${c.prefixCls}-treenode-unselectable`]: isUnselectable.value,
    [`${c.prefixCls}-treenode-switcher-${props.expanded ? 'open' : 'close'}`]: !memoizedIsLeaf.value,
...
const wrapClass = computed(() => `${ctx.value?.prefixCls || 'vc-tree'}-node-content-wrapper`)
const selectorClass = computed(() => {
  const c = ctx.value
  if (!c) return ''
  return clsx(wrapClass.value, `${wrapClass.value}-${nodeState.value || 'normal'}`, {
    [`${c.prefixCls}-node-selected`]: !isDisabled.value && (props.selected || dragNodeHighlight.value),
  })
})
...
const showLoadingIcon = computed(() => !showIcon.value && !!ctx.value?.loadData && !!props.loading)
...
const iconEleStyle = computed<CSSProperties>(() => ctx.value?.styles?.itemIcon || {})
...
const titleStyle = computed<CSSProperties>(() => ctx.value?.styles?.itemTitle || {})

const dropIndicatorData = computed(() => {
  const c = ctx.value
  if (!c) return null
  const rootDraggable = Boolean(c.draggable)
  if (!(!props.disabled && rootDraggable && c.dragOverNodeKey === props.eventKey))
    return null
  if (c.dropPosition === null || c.dropLevelOffset === null || c.indent === null)
    return null
...
const ariaSelected = computed(() => {
  if (isSelectable.value && !isDisabled.value)
