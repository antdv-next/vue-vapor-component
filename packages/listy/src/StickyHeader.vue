...
type GetSizeFn = (startKey: any, endKey?: any) => { top: number; bottom: number }
...
const currGroupItems = computed(() => props.groupKeyToItems.get(currGroupKey.value) || [])
...
const top = computed(() => {
  ...
  return Math.min(0, props.getSize(toTaggedKey(nextGroupKey, 'group')).top - headerHeight.value - props.scrollTop)
})
...
<template>
  <Portal v-if="container" :open="true" :get-container="() => container!">
    <div :class="`${props.prefixCls}-group-header-holder`">
      <GroupHeader
        :group="props.group"
        ...
        fixed
        :class-name="props.headerClassName"
