...
function getTrackNode(trackRef: any) {
  ...
  // Original VC JSX uses trackRef.node (createRef).
  // Vue shallowRef stores the element in .value.
  if (typeof trackRef.value !== 'undefined' && trackRef.value !== null) {
    return trackRef.value
  }
  if (trackRef.node) {
    return trackRef.node
  }
  // If it's already a raw HTMLElement, return as-is
  if (typeof trackRef === 'object' && 'clientWidth' in trackRef) {
    return trackRef
  }
  return null
}
