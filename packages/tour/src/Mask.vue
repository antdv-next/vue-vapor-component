<script setup vapor lang="ts">
  ...
  const mergedAnimated = computed(() =>
    typeof props.animated === 'object' ? props.animated?.placeholder : !!props.animated,
  )
  ...
  const maskRectSize = computed(() => isSafari.value
...
<template>
  <Portal :open="open" :auto-lock="!inlineMode" :get-container="getPopupContainer" :on-esc="onEsc">
    <div
    ...
    >
      <svg v-if="showMask" style="width: 100%; height: 100%">
        <defs>
          <mask :id="maskId">
            <rect x="0" y="0" :width="maskRectSize.width" :height="maskRectSize.height" fill="white" />
            <rect
              v-if="pos"
              :x="pos.left"
              :y="pos.top"
